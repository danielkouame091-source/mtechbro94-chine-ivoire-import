...
# =========================================================
# FONCTIONS MÉTIER : RISQUE, SURESTARIES & INNOVATIONS
# =========================================================
...
def attribuer_bureau_anonyme(dossier_id):
    bureaux_virtuels = [
        "Bureau Virtuel Nord (Korhogo)",
        "Bureau Virtuel Ouest (Man)",
        "Bureau Virtuel Centre (Yamoussoukro)",
        "Bureau Virtuel Maritime (San-Pédro)"
    ]
...
<div class="header-banner">
    ...
    <p>Module Intégré : Cargo, Sélectivité Douanière, Facturation Client, Surestaries, EDI, Innovations Inde/Chine & IA</p>
</div>
""", unsafe_allow_html=True)
...
tabs_list = [
    ...
    "🌐 7. Innovations Inde & Chine",
    "📖 8. Code des Douanes",
    "🤖 9. Assistant IA Transit",
    "🔐 Admin & Audit Logs"
]
...
# =========================================================
# TAB 7 : INNOVATIONS INDE & CHINE (FACELESS & OEA)
# =========================================================
with tab_innov:
    st.markdown('<div class="custom-card-3d">', unsafe_allow_html=True)
    st.subheader("🌐 Modules d'Innovation Inspirés des Modèles Indien (ICEGATE) & Chinois (Smart Customs)")
    st.caption("Dédouanement Anonymisé (Faceless Assessment), Certification OEA & Moteur de Valeur Prédictif")

    t_i1, t_i2, t_i3 = st.tabs(["🔒 1. Faceless Assessment (Inde)", "🏆 2. Certification OEA / AEO", "🔍 3. Moteur IA de Valeur (Chine)"])

    # ---------------------------------------------------------
    # SOUS-TAB 1 : FACELESS ASSESSMENT (ANONYMISATION)
    # ---------------------------------------------------------
    with t_i1:
        st.markdown("#### 🔒 Dédouanement Anonymisé & Impartial (Modèle Turant Customs)")
        st.info("Le système masque les identités de l'importateur et du transitaire. Le dossier est dispatché vers un inspecteur distant choisi aléatoirement pour supprimer tout risque de connivence.")

        conn = sqlite3.connect(DB_NAME)
        df_d_face = pd.read_sql_query("SELECT id, date, article, regime, fob_xof, canal_selectivite FROM dossiers ORDER BY id DESC LIMIT 5", conn)
        conn.close()
        ...
        if not df_d_face.empty:
            ...
            with col_fa1:
                st.markdown("##### 📄 Données Anonymisées pour l'Inspecteur")
                st.json({
                    "Dossier_ID": f"SAD-2026-{row_f['id']}",
                    "Importateur": "******** [MASQUÉ / CONFIDENTIEL]",
                    "Transitaire": "******** [MASQUÉ / CONFIDENTIEL]",
                    "Marchandise": row_f['article'],
                    "Valeur_FOB_XOF": f"{row_f['fob_xof']:,.0f} FCFA",
                    "Régime": row_f['regime']
                })
            with col_fa2:
                st.markdown("##### 🏛️ Affectation Impartiale")
                st.success(f"**Bureau Répartiteur :** `{bureau_assigne}`")
                st.caption("L'inspecteur affecté ne connaît ni l'entreprise ni le lieu de dépôt physique de la marchandise.")
                st.button("✅ Valider l'Évaluation Anonyme", key=f"btn_fa_{sel_f_id}")
        else:
            st.info("Aucun dossier enregistré pour le contrôle anonyme.")

    # ---------------------------------------------------------
    # SOUS-TAB 2 : PROGRAMME OEA / AEO (TRUST SCORE)
    # ---------------------------------------------------------
    with t_i2:
        st.markdown("#### 🏆 Programme Opérateur Économique Agréé (OEA / AEO)")
        st.caption("Système de confiance à 3 niveaux réduisant le taux d'inspection à moins de 2% pour les entreprises certifiées.")

        col_oea1, col_oea2 = st.columns(2)
        with col_oea1:
            nom_ent = st.text_input("Nom de l'Entreprise à Évaluer", value="SOCIETE IVOIRIENNE DE NEGOCE")
            ...
            nb_litiges = st.number_input("Nombre de Contentieux / Infractions (3 ans)", value=0)

            tier_oea, canal_recommande, taux_insp = evaluer_statut_oea(historique_conformite, anciennete_ans, nb_litiges)

        with col_oea2:
            st.markdown("##### 📊 Résultat d'Évaluation OEA")
            st.markdown(f"**Statut Certifié :** `{tier_oea}`")
            st.markdown(f"**Traitement Douanier :** {canal_recommande}")
            st.metric("Taux d'Inspection Physique Appliqué", f"{taux_insp * 100:.1f} %")

    # ---------------------------------------------------------
    # SOUS-TAB 3 : MOTEUR IA DE VALEUR & SH (SMART CUSTOMS CHINE)
    # ---------------------------------------------------------
    with t_i3:
        st.markdown("#### 🔍 Moteur d'Auto-Classification SH & Contrôle de Valeur")
        st.caption("Analyse NLP des libellés de factures et comparaison avec la base de prix mondiale.")
        ...
        if st.button("🧠 Analyser par l'IA de Nomenclatures", use_container_width=True):
            st.success("Code SH Suggéré par IA : **8471.30.00** (Machines automatiques de traitement de l'information)")
            st.info("💡 **Analyse de Valeur :** Prix déclaré ($ 2,400 USD) conforme à la fourchette du marché mondial ($2,200 -$ 2,600 USD). Aucun risque de sous-évaluation détecté.")

    st.markdown('</div>', unsafe_allow_html=True)

# =========================================================
# TAB 8 : CODE DES DOUANES
# =========================================================
with tab_code:
    st.markdown('<div class="custom-card-3d">', unsafe_allow_html=True)
...
# =========================================================
# TAB 9 : ASSISTANT IA TRANSIT & DOUANES
# =========================================================
with tab_ai:
    st.markdown('<div class="custom-card-3d">', unsafe_allow_html=True)
