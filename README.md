...
DB_NAME = str(BASE_DIR / "transit_enterprise.db")
...
def get_articles_db() -> pd.DataFrame:
    conn = sqlite3.connect(DB_NAME)
    df = pd.read_sql_query("SELECT * FROM articles ORDER BY nom ASC", conn)
    conn.close()
    return df
...
def ajouter_dossier_db(client, article, fob, total, solde, statut="En cours", doc_path=""):
    ...
    cursor.execute(
        ...
        (datetime.now().strftime("%Y-%m-%d %H:%M"), client, article, fob, total, solde, statut, doc_path),
    )
    conn.commit()
    conn.close()


def get_dossiers_db() -> pd.DataFrame:
    conn = sqlite3.connect(DB_NAME)
...
def get_default_users():
    return {
        "admin": {"password": get_setting("APP_ADMIN_PASSWORD", "transit2026"), "role": "Administrateur"},
        "commercial": {"password": get_setting("APP_COMMERCIAL_PASSWORD", "compta2026"), "role": "Commercial / Déclarant"},
        "comptable": {"password": get_setting("APP_COMPTABLE_PASSWORD", "finance2026"), "role": "Comptable / Trésorerie"},
    }
...
def generer_pdf_devis_pro(nom_client, article_nom, item_info, quantite, fob_xof, fret_xof, assurance_xof, caf_xof, total_douane, total_transit, post_acheminement, total_facture, acompte, solde_du):
    safe_name = re.sub(r"[^A-Za-z0-9_-]+", "_", nom_client.strip()) or "client"
    pdf_filename = BASE_DIR / f"Devis_Pro_{safe_name}.pdf"
    ...
    elements.append(
        Paragraph(
            "Agrément Douane N° 2026/CI-ABJ | Abidjan Port & San-Pédro<br/>Contact : contact@kelanewin-transit.ci | Tél: +225 07 00 00 00 00",
            subtitle_style,
        )
    )
    ...
    info_client_text = (
        f"<b>Client / Importateur :</b> {nom_client}<br/>"
        f"<b>Date d'émission :</b> {datetime.now().strftime('%d/%m/%Y')}<br/>"
        f"<b>Objet :</b> Facture Proforma & Cotation Logistique Douanière"
    )
    ...
    elements.append(
        Paragraph(
            "<b>Conditions de règlement :</b> 50% à la commande, solde avant BAE (Bon à Enlever).<br/>"
            "<i>Arrêtée la présente proforma à la somme de <b>{:,.0f} FCFA</b>. Cachet & Signature autorisés :</i>".format(total_facture),
            styles["Normal"],
        )
    )
...
st.set_page_config(page_title="Kelanewin Transit - SYDAM Pro Enterprise", page_icon="🇨🇮", layout="wide")

st.markdown(
    """
    <style>
    .stApp { background-color: #0B0F19; color: #F8FAFC; font-family: 'Inter', system-ui, -apple-system, sans-serif; }
    .header-banner { background: linear-gradient(135deg, #047857 0%, #10B981 50%, #0284C7 100%); padding: 30px; border-radius: 20px; color: white; margin-bottom: 25px; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.5), 0 8px 10px -6px rgba(0, 0, 0, 0.3); border: 1px solid rgba(255,255,255,0.15); }
    .custom-card-3d { background: #1E293B; border-radius: 18px; padding: 25px; border: 1px solid #334155; margin-bottom: 25px; box-shadow: 8px 8px 16px #070a11, -8px -8px 16px #151a27; }
    .kpi-card { background: linear-gradient(145deg, #1e293b, #111827); border-radius: 14px; padding: 18px; border: 1px solid #374151; box-shadow: inset 1px 1px 2px rgba(255,255,255,0.05), 0 10px 15px -3px rgba(0,0,0,0.3); text-align: center; }
    .kpi-title { font-size: 0.85rem; color: #9CA3AF; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 5px; }
    .kpi-value { font-size: 1.4rem; font-weight: 700; color: #38BDF8; }
    .profit-box-3d { background: linear-gradient(135deg, #15803D 0%, #166534 100%); color: white; padding: 22px; border-radius: 16px; text-align: center; margin-top: 15px; box-shadow: inset 2px 2px 5px rgba(255,255,255,0.2), 0 10px 20px rgba(21, 128, 61, 0.4); font-size: 1.25rem; font-weight: 600; }
    .ai-box-3d { background: linear-gradient(135deg, #1E1B4B 0%, #312E81 100%); border: 1px solid #6366F1; padding: 22px; border-radius: 16px; margin-top: 15px; box-shadow: 0 10px 20px rgba(99, 102, 241, 0.25); }
...
if not st.session_state.authenticated:
    ...
    with center:
        ...
        if st.button("Se connecter", use_container_width=True):
            utilisateurs = get_default_users()
            if username_input in utilisateurs and utilisateurs[username_input]["password"] == password_input:
                ...
                st.error("Identifiants erronés. Modifiez les variables d'environnement ou les secrets pour sécuriser votre accès.")
...
if not groq_api_key:
    groq_api_key = st.sidebar.text_input("🔑 Clé API Groq", value="", type="password")
else:
    groq_api_key = st.sidebar.text_input("🔑 Clé API Groq", value=groq_api_key, type="password")
...
smtp_server = st.sidebar.text_input("Serveur SMTP", value=get_setting("SMTP_SERVER", "smtp.gmail.com"))
smtp_port = st.sidebar.number_input("Port SMTP", value=int(get_setting("SMTP_PORT", "587")))
sender_email = st.sidebar.text_input("Email Expéditeur", value=get_setting("SMTP_USERNAME", ""))
smtp_password = st.sidebar.text_input("Mot de passe application", value=get_setting("SMTP_PASSWORD", ""), type="password")
...
liste_articles_noms = df_articles_db["nom"].tolist() if not df_articles_db.empty else []
...
with tab_cotation:
    ...
    uploaded_file = st.file_uploader(
        "📎 Joindre les pièces justificatives (Facture Proforma, B/L, Packing List en PDF ou Image)",
    ...
    if uploaded_file is not None:
        ...
        saved_doc_path = str(UPLOAD_DIR / f"{datetime.now().strftime('%Y%m%d%H%M%S')}_{safe_name}")
        with open(saved_doc_path, "wb") as file:
            file.write(uploaded_file.getbuffer())
        st.success(f"Document joint avec succès : {uploaded_file.name}")

    st.markdown("</div>", unsafe_allow_html=True)

    if not df_articles_db.empty:
        st.markdown('<div class="custom-card-3d">', unsafe_allow_html=True)
        ...
        pdf_path = generer_pdf_devis_pro(
            nom_client,
            article_nom,
            item_info,
            quantite,
        ...
        msg_texte = (
            ...
            f"Veuillez trouver ci-joint la cotation officielle & proforma de Kelanewin Transit pour votre article {article_nom}.\n"
            f"Montant Total : {total_facture:,.0f} FCFA. Solde dû : {solde_du:,.0f} FCFA.\n"
                ...
                if not sender_email or not smtp_password:
                    ...
                    try:
                        ...
                        with open(pdf_path, "rb") as file:
                            attach = MIMEApplication(file.read(), Name=Path(pdf_path).name)
                        attach["Content-Disposition"] = f'attachment; filename="{Path(pdf_path).name}"'
                        msg.attach(attach)
                        ...
                        st.error(f"Erreur d'envoi SMTP : {exc}")
        ...
        st.warning("Aucun article n'est actuellement disponible dans la base SQLite. Ajoutez-en un dans l'onglet Base de Données SH.")
    ...
    if df_dossiers.empty:
        ...
        with col_w2:
            nouveau_statut = st.selectbox(
                ...
                ["En cours", "FDI & RFC Validées", "Visite Douanière en Cours", "Bon à Enlever (BAE) Émis", "Livré au Client"],
            )
        with col_w3:
            st.markdown("<br/>", unsafe_allow_html=True)
...
with tab_ai_expert:
    ...
    st.subheader("🤖 Assistant Expert Kelanewin Transit (Groq)")
    ...
    if st.button("🔍 Interroger l'Expert"):
        if groq_api_key and Groq:
            try:
                ...
                st.error(f"Erreur d'initialisation Groq : {exc}")
        else:
            st.warning("Veuillez renseigner votre clé API Groq dans la barre latérale ou dans vos variables d'environnement.")
    st.markdown("</div>", unsafe_allow_html=True)
        ...
        with st.form("form_ajout_article"):
            ...
            if submit_article and n_nom and n_sh:
                try:
                    ...
                    st.error(f"Erreur (l'article existe peut-être déjà) : {exc}")
    else:
